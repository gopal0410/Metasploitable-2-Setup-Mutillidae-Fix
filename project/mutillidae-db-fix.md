...
This error states that there is no such table as `accounts` in the database `metasploite`.

![db-error](https://github.com/gopal0410/Metasploitable-2-Setup-Mutillidae-Fix/blob/master/images/db-error.png)

## Checking MySQL

After finding out that the issue is with the table that does not exist. Then we begin by logging into MySQL in the VM and checking the databases using
`SHOW DATABASES;`

![db-confirm](https://github.com/gopal0410/Metasploitable-2-Setup-Mutillidae-Fix/blob/master/images/db-confirm.png)

After getting the list of databases in MySQL, we realize that the `metasploit` database does exist, so we check the tables within it by using the following commands
`USE metasploit;`
`SHOW TABLES;`
...
Now, since we know that there is a database error caused by the lack of a table and that there is another database called `owasp10` which holds the actual table, we change the database that is being used in the `config.inc` file and change it from `metasploit` to `owasp10`. 
